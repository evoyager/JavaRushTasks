       ...
       class DogPet extends SuperDog implements Sayable {
           ...
           public String say() {
               if (i < 1) {
                   ...
                   StringBuilder gaf = new StringBuilder("г");
                   for (int j = 0; j < i; j++) {
                       gaf.append("а");
                   }
                   gaf.append("в");
                   String date = formatter.format(new Date());
                   return String.format("%s лает %s! %s", getName(), gaf, date);
               }
           }

           private String getName() {
               StringBuilder builder = new StringBuilder(getSuperQuotes()).append(name).append(getSuperQuotes());
